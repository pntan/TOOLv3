...
(function() {
    ...
    // =========================================================================
    // KHU VỰC QUẢN LÝ CẤU HÌNH (Đã sửa lỗi Vòng lặp Vô hạn)
    // =========================================================================
    ...
    /**
     * @func setConfig (ĐÃ SỬA LỖI VÒNG LẶP VÔ HẠN)
     * @description 'Ghi cấu hình'
     */
    var setConfig = (config_name, config_value) => {
        // setItem tự động ghi đè, không cần kiểm tra if/else hay gọi lại hàm.
        localStorage.setItem(`TP_CONFIG_${config_name}`, JSON.stringify(config_value));
    }
    
    // =========================================================================
    // HÀM LẤY THÔNG TIN (ĐÃ THÊM LẠI AUTHORIZATION)
    // =========================================================================
    ...
    async function getUrlServer(owner = "pntan", repo = "TOOLv3", path = "version", branch = "main") {
        try {
            // Thêm lại Authorization Token theo yêu cầu
    ...
    async function getInfoPage (){
        ...
        // ... (Lược bỏ chi tiết các mục storage, browser, screen, document, window... để code gọn hơn)
        ...
        func_list.forEach(el => {
            ...
            if(shouldDisplay){
                ...
                $(".tp-container.tp-main .content-screen .screen.screen-main .list-function").append(`
                    <div class="box-function" data-func="${el.func_name}" data-layout="${el.layout_name}">
                        <p>${el.name}</p>
                    </div>
                `);
            }
    ...
    var INIT_FUNCTION = async () => {
        ...
        // Chọn chức năng
        $(".tp-container.tp-main .content-screen .screen.screen-main .list-function .box-function").on("click", function(e){
            var func = $(this).attr("data-func") || null;
						var layout = $(this).attr("data-layout").length > 0 ?  layout = $(this).attr("data-layout") : null;
						...
						if(layout){
							...
							$(`.tp-container.tp-main .content-screen .screen.screen-main .layout-function .box#${layout}_layout`).addClass("show");
						}
        })
				...
				$(".tp-container.tp-main .content-screen .screen.screen-main .layout-function .back").on("click", function(){
					$(".tp-container.tp-main .content-screen .screen.screen-main .layout-function .box").remove("show");
        ...
        $("body").on("mousemove", function(e){
            ...
            if(x <= X_LIMIT) {
                $(".tp-container.tp-main").css({"left": "0", "right": ""}).addClass("active");
            } else if(X_LIMIT >= bodyWidth - x){
                $(".tp-container.tp-main").css({"right": "0", "left": ""}).addClass("active");
            } else {
    ...
    // Hàm thử nghiệm
    var funcTest = () => {
        boxAlert("Hàm thử nghiệm ĐÃ CHẠY");
    }

		var gia_duoi = () => {
			boxAlert("Chức năng sửa giá đuôi");
		}
